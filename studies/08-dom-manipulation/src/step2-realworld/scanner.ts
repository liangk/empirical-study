...
import { detectDomPatterns, DomFinding } from '../step3-static-analysis/detector/dom-pattern-detector';
...
function findSourceFiles(repoDir: string): string[] {
  return glob.sync('**/*.{js,jsx,ts,tsx,mjs,vue}', {
    cwd: repoDir,
    absolute: true,
    ignore: [
...
async function main() {
  ...
  for (let i = 0; i < corpus.length; i++) {
    ...
    process.stdout.write(`${tag} (${repo.framework}) ... `);
    ...
    try {
      ...
      for (const file of files) {
        const findings: DomFinding[] = detectDomPatterns(file);
        for (const f of findings) {
          const serialized = JSON.stringify({ ...f, repo: `${repo.owner}/${repo.name}`, framework: repo.framework });
          ...
          prevalence.byFramework[repo.framework] = (prevalence.byFramework[repo.framework] ?? 0) + 1;
        }
      }
  ...
  for (const [pattern, count] of Object.entries(prevalence.byPattern).sort((a, b) => b[1] - a[1])) {
    const pct = prevalence.totalFindings > 0 ? ((count / prevalence.totalFindings) * 100).toFixed(1) : '0.0';
    console.log(`  ${pattern}: ${count} (${pct}%)`);
  }
  console.log(`\nBy framework:`);
  for (const [fw, count] of Object.entries(prevalence.byFramework).sort((a, b) => b[1] - a[1])) {
