...
function printGrid<X, Y>(exp: ExperimentResult<X, Y>, metrics: MetricKey[]): void {
  console.log(`\n${'═'.repeat(72)}`);
  console.log(`  [BM-05] ${exp.caseName}`);
...
function buildJsonOutput<X, Y>(exp: ExperimentResult<X, Y>, metrics: MetricKey[]): any {
  ...
  return { caseId: exp.caseId, caseName: exp.caseName, module: 'BM-05', xAxisName: exp.xAxisName, yAxisName: exp.yAxisName, tables, timestamp: exp.timestamp };
}
...
async function main(): Promise<void> {
  ...
  console.log('╔══════════════════════════════════════════════════════════════════════╗');
  console.log('║   BM-05: Timer Leak — 2D Impact Experiments                         ║');
  console.log('╚══════════════════════════════════════════════════════════════════════╝');

  const allResults: any[] = [];

  if (!caseFlag || caseFlag === '1') {
    console.log('\n▶ Case 1: Leak Probability × Timer Creation Rate');
    const r = runBm05Case1();
  ...
  if (!caseFlag || caseFlag === '3') {
    console.log('\n▶ Case 3: Timer Interval × Creation Rate');
    const r = runBm05Case3();
    printGrid(r, ['totalCallbackInvocations', 'leakedConnections', 'heapGrowthBytes', 'meanLatencyMs']);
    allResults.push(buildJsonOutput(r, ['totalCallbackInvocations', 'leakedConnections', 'heapGrowthBytes', 'meanLatencyMs']));
  }
  if (!caseFlag || caseFlag === '4') {
    console.log('\n▶ Case 4: Timer Type × Leak Probability');
    const r = runBm05Case4();
    printGrid(r, ['leakedConnections', 'heapGrowthBytes', 'totalCallbackInvocations', 'timeToExhaustion']);
    allResults.push(buildJsonOutput(r, ['leakedConnections', 'heapGrowthBytes', 'totalCallbackInvocations', 'timeToExhaustion']));
  }

  const outFile = path.join(RESULTS_DIR, `experiments-bm05-${timestamp}.json`);
