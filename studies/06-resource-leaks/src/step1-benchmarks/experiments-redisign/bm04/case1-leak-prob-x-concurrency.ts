/**
 * BM-04 Case 1: Leak Probability × Concurrency
 *
 * Shows how http.request() without req.destroy() on error/timeout paths
 * exhausts socket limits under parallel traffic. Mirrors BM-01 Case 1.
 * X-axis: leak probability (0% to 20%)
 * Y-axis: concurrent outgoing HTTP requests
 * Metrics: failure rate, time-to-socket-exhaustion, throughput, leaked sockets
 */
import { ExperimentResult, GridCell } from '../types';
import { SocketConfig, SocketWorkloadConfig, runSocketSimulation } from '../socket-simulator';

const CASE_ID = 'bm04-case1';
const CASE_NAME = 'Leak Probability × Concurrency';

const leakProbs = [0, 0.01, 0.02, 0.05, 0.10, 0.20];
const concurrencies = [1, 5, 10, 20, 50, 100];

const baseConfig: SocketConfig = { maxSockets: 50, responseTimeMs: 50, responseTimeJitter: 10 };

export function runBm04Case1(): ExperimentResult<number, number> {
  const grid: GridCell<number, number>[][] = [];

  for (const conc of concurrencies) {
    const row: GridCell<number, number>[] = [];
    for (const lp of leakProbs) {
      const workload: SocketWorkloadConfig = {
        durationMs: 30_000, concurrency: conc,
        arrivalIntervalMs: Math.max(1, Math.round(50 / conc)),
        timeoutMs: 5000, leakProbability: lp,
        errorRate: 0, destroyOnError: true,
        responseSize: 1024, keepAlive: false,
      };
      row.push({
        xParam: lp, yParam: conc,
        xLabel: `${(lp * 100).toFixed(0)}%`, yLabel: `${conc}`,
        result: runSocketSimulation(baseConfig, workload),
      });
    }
    grid.push(row);
  }

  return {
    caseId: CASE_ID, caseName: CASE_NAME,
    xAxisName: 'Leak Probability', yAxisName: 'Concurrency',
    xValues: leakProbs, yValues: concurrencies,
    grid, metric: 'failureRate', timestamp: new Date().toISOString(),
  };
}
