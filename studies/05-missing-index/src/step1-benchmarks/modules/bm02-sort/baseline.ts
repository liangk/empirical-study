...
export const bm02: BenchmarkModule = {
  ...
  async runBaseline(_n: number): Promise<number> {
    const db = getClient();
    return timeQuery(() =>
      db.$queryRaw`SELECT * FROM bench_orders ORDER BY created_at DESC LIMIT 20`
    );
  },
  ...
  async createIndex(): Promise<void> {
    const db = getClient();
    await db.$executeRawUnsafe(`CREATE INDEX IF NOT EXISTS ${INDEX_NAME} ON bench_orders(created_at DESC)`);
  },
  ...
  async verify(_n: number): Promise<boolean> {
    const db = getClient();
    const base = await db.$queryRaw<{ id: number }[]>`SELECT id FROM bench_orders ORDER BY created_at DESC LIMIT 20`;
    await this.createIndex();
    const opt = await db.$queryRaw<{ id: number }[]>`SELECT id FROM bench_orders ORDER BY created_at DESC LIMIT 20`;
  ...
  async explainPlanType(_n: number): Promise<string> {
    ...
    const result = await db.$queryRaw<any[]>`
      EXPLAIN (ANALYZE, FORMAT JSON) SELECT * FROM bench_orders ORDER BY created_at DESC LIMIT 20
    `;
